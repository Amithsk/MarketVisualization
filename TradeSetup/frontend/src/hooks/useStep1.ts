...
import { fetchStep1Preview, freezeStep1Context } from "@/services/step1.api";
...
export function useStep1(tradeDate: TradeDate) {
  ...
  const previewStep1 = useCallback(async () => {
    ...
    try {
      const response: Step1PreviewResponse = await fetchStep1Preview(tradeDate);
      setState({
  ...
  const freezeStep1 = useCallback(async () => {
    ...
    try {
      const response: Step1FrozenResponse = await freezeStep1Context(tradeDate);
      setState({
        data: response.snapshot,
        loading: false,
        error: null,
