...
import type { FrozenTrade, Step4FrozenTradeResponse } from "@/types/step4.types";
...
export function useStep4() {
   ...
   * Freeze final trade
   */
  const freezeTrade = useCallback(async (trade: FrozenTrade) => {
    setState((prev) => ({ ...prev, loading: true, error: null }));

    try {
      const response: Step4FrozenTradeResponse =
        await freezeFinalTrade(trade);
